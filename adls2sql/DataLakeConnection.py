...
class DatalakeConnnection():
    ...
    def get_column_names(
        ...
        if file_format == 'CSV':
            deserializer = CSVDeserializer(file_contents, first_row_as_header, file_format_options)
    ...
    def get_rows(
        ...
        if file_format == 'CSV':
            deserializer = CSVDeserializer(csv_contents, headers_in_first_row, file_format_options)
        elif file_format =='JSON':
    ...
    def cache_csv_dir_as_table(
        ...
        if self.is_path_a_file(path):
            self.cache_csv_file_as_table(file_format, container, path, cached_table_name, column_mappings, file_format_options)
        else:
            dir_list = self.list_files(container, path)
            for f in dir_list:
                self.cache_csv_file_as_table(file_format, container, f, cached_table_name, column_mappings, file_format_options)
    ...
    def execute_sql_dict_output(
        ...
        for m in mappings:
            self.cache_csv_dir_as_table(
                m.file_format,
                m.container_name,
                m.datalake_path,
                m.cached_table_name,
                m.column_mappings,
                m.file_format_options
            )
